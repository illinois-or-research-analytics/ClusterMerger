...
void ClusterMerger::WriteClusterQueue() {
    std::ofstream clustering_output(this->output_file);
    int current_cluster_id = 0;
    clustering_output << "node_id,cluster_id" << '\n';
    while(!this->output_cluster_queue.empty()) {
        std::vector<int> current_cluster = this->output_cluster_queue.front();
        this->output_cluster_queue.pop();
    ...
    this->WriteToLogFile("Output clustering written to " + this->output_file, Log::info);
}

int ClusterMerger::WriteToLogFile(std::string message, Log message_type) {
